...
import { Investment, InvestmentType, PatrimonyEvolution, Summary } from './models'
...
export function OverviewComponent() {
	...
	const walletTableSource = Object.entries(investmentsByType ?? {}).map(([key, value]) => ({
		key: key,
		type: key,
		participacao: formatPercentage(
			(value?.reduce((acc, val) => acc + val.Value, 0) ?? 0) / (summary?.TotalValue ?? 1),
		),
		...
		<div id="OverviewComponent">
			<section className="CardSection">
				<Card title="My Summary" className="OverviewCard SummaryCard">
					<Statistic title="Net Worth" value={summary.TotalValue} precision={2} prefix={'R$'} />
					...
					/>
					<Divider className="Divider" />
					<Statistic title="Gain in month" value={summary.MonthGain} precision={2} prefix={'R$'} />
					<Divider className="Divider" />
					<Statistic
				...
				<Card title="My Wallet" className="OverviewCard WalletCard">
					<div className="WalletWrapper">
						...
						<div className="WalletTable">
							<Table dataSource={walletTableSource} columns={walletTablecolumns} />
						</div>
					</div>
				</Card>
			</section>
